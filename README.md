...
| rootCause | 第一選択 |
|-----------|----------|
| **TEXT**（誤字・文字崩れ） | **Smart Auto Fix** + OpenAI 再生成（Nano Banana 非推奨） |
...
**TEXT**（誤字・日本語崩れ・文字欠けなど）は Nano Banana では修正しません。改善スクリプトは **自動的にスキップ** し、Smart Auto Fix + OpenAI 再生成を使ってください。

```bash
npm run smart-auto-fix          # 原因別の改善計画（dry-run）
npm run smart-auto-fix -- --apply   # slide / プロンプトへ指示追記
npm run image-improve           # 不合格スライドの OpenAI 再生成
```
