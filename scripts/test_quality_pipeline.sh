...
echo "-- Test 98: VERSION updated to v1.86.13 --"
node --input-type=module <<'EOF'
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
...
if (
  ...
  throw new Error("docs/VERSION.md current version must be v1.86.13");
}
...
if (
  ...
  throw new Error("docs/VERSION.md current version must not remain v1.86.12");
}

console.log("VERSION v1.86.13 ok");
EOF
pass "VERSION updated to v1.86.13"
...
echo "-- Test 1232: v1.86.13 current version metadata in VERSION.md --"
node --input-type=module <<'EOF'
import fs from "node:fs";
...
if (
  ...
  throw new Error("current VERSION section must declare v1.86.13");
}
...
for (const marker of [
  ...
  "**1232 PASS**",
  "**v1.86.14** v1.86.13 released-state reconciliation — **Implementation** / **Not Declared**",
...
if (
  ...
  currentSection.includes("v1.86.13 Commit / Tag / Push **Pending**") ||
  currentSection.includes("**Next Phase Candidate:** Commit Execution for **v1.86.13**") ||
  currentSection.includes(
    "**Repository Baseline Commit:** `881081a1c037093ad275ac0d3c8a1362cc9e017d`",
  ) ||
  currentSection.includes("**Repository Baseline Tag:** `v1.86.12`") ||
  currentSection.includes(
    "**v1.86.14** v1.86.13 released-state reconciliation — **Planning**",
  ) ||
  currentSection.includes("corrective **v1.86.14** released-state reconciliation — current phase **Planning**") ||
  currentSection.includes("Pending Phase: Planning")
) {
  throw new Error(
    "current VERSION section must not treat v1.86.12 released-state reconciliation as sole current, leave Pending v1.86.13 publication claims, leave Record commit/tag at v1.86.12, or affirm Planning as active pending phase for v1.86.14",
  );
}

console.log("v1.86.13 current version metadata ok");
EOF
pass "v1.86.13 current version metadata in VERSION.md"
