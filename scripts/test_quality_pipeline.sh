...
echo "-- Test 98: VERSION updated to v1.86.20 --"
node --input-type=module <<'EOF'
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
...
if (
  ...
  throw new Error("docs/VERSION.md current version must be v1.86.20");
}
...
if (
  ...
  throw new Error("docs/VERSION.md current version must not remain v1.86.19");
}

console.log("VERSION v1.86.20 ok");
EOF
pass "VERSION updated to v1.86.20"
...
echo "-- Test 1232: v1.86.20 current version metadata in VERSION.md --"
node --input-type=module <<'EOF'
import fs from "node:fs";
...
if (
  ...
  throw new Error("current VERSION section must declare v1.86.20");
}
...
if (
  ...
  currentSection.includes("v1.86.20 Commit / Tag / Push **Pending**") ||
  currentSection.includes("**Next Phase Candidate:** Commit Execution for **v1.86.20**") ||
  currentSection.includes(
    "**Repository Baseline Commit:** `70771c2996a8226b628e90fffe36ca0aae8e0363`",
  ) ||
  currentSection.includes("**Repository Baseline Tag:** `v1.86.19`") ||
  currentSection.includes(
    "**v1.86.21** v1.86.20 released-state reconciliation — **Planning**",
  ) ||
  currentSection.includes("corrective **v1.86.21** released-state reconciliation — current phase **Planning**") ||
  currentSection.includes("Pending Phase: Planning")
) {
  throw new Error(
    "current VERSION section must not treat v1.86.19 released-state reconciliation as sole current, leave Pending v1.86.20 publication claims, leave Record commit/tag at v1.86.19, or affirm Planning as active pending phase for v1.86.21",
  );
}

console.log("v1.86.20 current version metadata ok");
EOF
pass "v1.86.20 current version metadata in VERSION.md"
