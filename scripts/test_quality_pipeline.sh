...
echo "-- Test 98: VERSION updated to v1.86.17 --"
node --input-type=module <<'EOF'
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
...
if (
  ...
  throw new Error("docs/VERSION.md current version must be v1.86.17");
}
...
if (
  ...
  throw new Error("docs/VERSION.md current version must not remain v1.86.16");
}

console.log("VERSION v1.86.17 ok");
EOF
pass "VERSION updated to v1.86.17"
...
echo "-- Test 1232: v1.86.17 current version metadata in VERSION.md --"
node --input-type=module <<'EOF'
import fs from "node:fs";
...
if (
  ...
  throw new Error("current VERSION section must declare v1.86.17");
}
...
if (
  ...
  currentSection.includes("v1.86.17 Commit / Tag / Push **Pending**") ||
  currentSection.includes("**Next Phase Candidate:** Commit Execution for **v1.86.17**") ||
  currentSection.includes(
    "**Repository Baseline Commit:** `0132eb635db1da36eaaf027f69f66e37c5fa9232`",
  ) ||
  currentSection.includes("**Repository Baseline Tag:** `v1.86.16`") ||
  currentSection.includes(
    "**v1.86.18** v1.86.17 released-state reconciliation — **Planning**",
  ) ||
  currentSection.includes("corrective **v1.86.18** released-state reconciliation — current phase **Planning**") ||
  currentSection.includes("Pending Phase: Planning")
) {
  throw new Error(
    "current VERSION section must not treat v1.86.16 released-state reconciliation as sole current, leave Pending v1.86.17 publication claims, leave Record commit/tag at v1.86.16, or affirm Planning as active pending phase for v1.86.18",
  );
}

console.log("v1.86.17 current version metadata ok");
EOF
pass "v1.86.17 current version metadata in VERSION.md"
