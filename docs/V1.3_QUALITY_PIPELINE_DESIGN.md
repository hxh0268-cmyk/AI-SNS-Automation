...
### 10.4 運用パッチ（v1.3.1 実装済み）

| 項目 | 内容 |
|------|------|
| `src/lib/pipeline_workspace.js` | `archiveLatestOutput` / `cleanLatestOutput` / `preparePipelineWorkspace` |
| `--clean-latest` | 実行前に `reports/quality-pipeline/latest` を削除 |
| デフォルト | 既存 `latest` がある場合、`reports/quality-pipeline/archive/YYYY-MM-DD-HHmmss/` へ退避してから上書き |
| `report.md` | Next Actions（stopReason 別）、API キー設定案内、output 副産物 git 注意 |
| テスト | `test_quality_pipeline.sh` に clean-latest / report セクション確認を追加 |

`run_daily.sh` は変更しない。smart_auto_fix / openai_regenerate の実接続は v1.4 以降。
