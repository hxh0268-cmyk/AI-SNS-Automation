...
| フィールド | 説明 |
|------------|------|
| **Provider Name** | 一意識別子（例: `mock-text-generation` — **conceptual contract identity only**; not a `providerContracts[]` catalog registration ID authorization — see ADR-0016 / v1.72.0 catalog validator policy） |
