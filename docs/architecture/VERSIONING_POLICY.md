...
（historical Migration 5 note: editing this policy did not itself declare `v1.86.0`; `v1.86.0` is now **Released**; Pending corrective release is `v1.86.1`）。
...
- Release Declaration（本書編集 alone では宣言しない; `v1.86.0` is now Released via Git/Record; Pending corrective is `v1.86.1`）
