...
### Catalog Scope（v1.66.0）

| Scope | In Catalog | Authority |
|-------|------------|-----------|
| Application Layer `extract*PublicContract()` | **Yes** | Current catalog |
| Future Layer / Interaction / Cross Layer Design contracts | **No** — by design | Markdown Design Only until ADR + Compatibility Review |
| Runtime machine-readable schemas | **No** | Deferred |

Future Layer implementation MUST NOT bypass catalog governance. Catalog scope extension requires **Accepted ADR** — see [FUTURE_ENTRY_CRITERIA.md](./FUTURE_ENTRY_CRITERIA.md) §Public Contract Catalog Scope.
