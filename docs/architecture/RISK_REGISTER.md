...
| CL-013 | Public Contract traceability gap（Future Layer） | Catalog / Policy | High | Medium | ADR-0011 + ADR-0012 + ADR-0015 + ADR-0017 + v1.76.0 + v1.84.0 concrete mock registration | **Mitigated** — abstract + two governed concrete mocks in JSON（v1.84.0） | Real Provider impl still gated |

**Risk status:** Documented and mitigated at governance level — **not resolved** merely by documentation existence. CL-004, CL-005, CL-006 remain **open exposure** for cross-layer / side-effecting scope — **NOT APPLICABLE** for bounded side-effect-free Mock Provider formal assessment（**READY**）and **NOT APPLICABLE** for `image-generation-mock-provider` bounded Expansion Entry Authorization（v1.80.0）and bounded Implementation Authorization（v1.81.0）— **remains Deferred** globally. CL-013 **reassessed** post-v1.84.0 — governed concrete text mock and image mock traceability **implemented** in catalog JSON（v1.76.0 text + v1.84.0 image）. PR-004 **reassessed** post–ADR-0019/0020/0021/0022 — closed-world two-ID validator implemented at v1.84.0; Provider Contracts **3**. PR-005 **reframed** post-ADR-0019/0020/0021/0022 — Production Readiness Assessed ≠ Assessment Decision **READY** ≠ Expansion Entry Authorized ≠ Implementation Authorized ≠ Catalog Registered ≠ Bounded Production Ready ≠ Global Production Ready. PR-006 **reassessed** post–ADR-0020/0021/0022 — bounded text mock catalog registration **complete**（v1.76.0）; image provider catalog registration **complete**（v1.84.0）; `src/lib/image_generation.js` ≠ `image-generation-mock-provider`. PPRR-F001 **CLOSED AS REMEDIATED FOR THE BOUNDED MOCK PROVIDER ASSESSMENT** — bounded closure preserved at v1.84.0.
...
| PR-004 | Public Contract Catalog bypass | Catalog / Policy | High | Low | ADR-0011 + ADR-0012 + ADR-0015 registration + bypass prohibition + ADR-0019 expansion registration gate B22 | **Low** — catalog registration executed（v1.72.0）; expansion gate recorded（v1.79.0） | Critical if bypassed |
| PR-005 | Implementation Ready mistaken for Production Implementation | NON_GOALS / G-25 / Maturity | Critical | Medium | ADR-0014 scope limits + ADR-0015 Catalog Extension + ADR-0016 Authorized vs Started + ADR-0017 Governed vs Registered + ADR-0019 state model | **Medium** — governance distinction explicit; Bounded READY ≠ Expansion Entry Authorized | Critical |
| PR-006 | Mock Provider semantic drift / Application mock conflation | Provider / Application Layer | High | Medium | ADR-0016 + ADR-0017 + ADR-0018 + ADR-0019 + ADR-0020 + ADR-0021 + ADR-0022 identity mapping — `image_generation.js` ≠ `image-generation-mock-provider` | **Medium** — image candidate identity mapping governed at catalog registration governance; implementation drift risk remains | Major |
