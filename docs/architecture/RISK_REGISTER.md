...
**Risk status:** Documented and mitigated at governance level — **not resolved** merely by documentation existence. CL-004, CL-005, CL-006 remain **open exposure** for cross-layer / side-effecting scope — **NOT APPLICABLE** for bounded side-effect-free Mock Provider formal assessment（**READY**）and **NOT APPLICABLE** for `image-generation-mock-provider` bounded Expansion Entry Authorization（v1.80.0）and bounded Implementation Authorization（v1.81.0）— **remains Deferred** globally. CL-013 **reassessed** post-v1.76.0 — governed concrete Mock Provider traceability **implemented** in catalog JSON; Real Provider traceability still gated. PR-004 **reassessed** post–ADR-0019/0020/0021 — Expansion Entry Authorization and Implementation Authorization do **not** permit catalog registration; Provider Contracts remain **2**. PR-005 **reframed** post-ADR-0019/0020/0021 — Formal Assessment **READY** ≠ Expansion Entry Authorized ≠ Implementation Authorized ≠ Catalog Registered ≠ Bounded READY ≠ Global Production Ready. PR-006 **reassessed** post–ADR-0020/0021 — bounded text mock catalog registration **complete**（v1.76.0）; `src/lib/image_generation.js` ≠ `image-generation-mock-provider`; application mock ≠ Provider mock distinction recorded for image candidate; planned implementation path documentation-only. PPRR-F001 **CLOSED AS REMEDIATED FOR THE BOUNDED MOCK PROVIDER ASSESSMENT** — bounded closure preserved at v1.81.0.
...
| PR-006 | Mock Provider semantic drift / Application mock conflation | Provider / Application Layer | High | Medium | ADR-0016 + ADR-0017 + ADR-0018 + ADR-0019 + ADR-0020 + ADR-0021 identity mapping — `image_generation.js` ≠ `image-generation-mock-provider` | **Medium** — image candidate identity mapping governed at entry and implementation authorization; implementation drift risk remains | Major |
