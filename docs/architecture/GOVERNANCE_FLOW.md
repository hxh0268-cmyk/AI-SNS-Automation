...
9. Migration 7（GOVERNANCE_FLOW Authority Correction）does **not** populate the Current Baseline Record by itself and does **not** execute repository-wide synchronization. Historical note: Migration 7 did not declare `v1.86.0`; `v1.86.0` is now **Released** at `57b3182…`. Pending corrective release is `v1.86.1`（Not Declared）.
...
- Migration 7 による本書更新は **それ自体では release 宣言ではない**（historical: did not declare `v1.86.0`; `v1.86.0` is now **Released**; Pending corrective is `v1.86.1`）
