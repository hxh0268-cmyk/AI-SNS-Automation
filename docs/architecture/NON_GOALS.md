...
> **Current Phase（v1.81.0）:** `image-generation-mock-provider` Implementation Authorization **Granted**（bounded）— implementation execution **Not Started** — catalog registration **Not Authorized** — bounded text Mock Formal Decision **READY** preserved — Production Ready **Not Declared**（global）。Real Provider / external IO は **禁止維持**。
...
- **image-generation-mock-provider Implementation Authorization Granted** — v1.81.0 ADR-0021（bounded）; implementation execution **Not Started**; catalog registration **Not Authorized**
