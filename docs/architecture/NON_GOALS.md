...
> **Current Phase（v1.80.0）:** `image-generation-mock-provider` Provider Expansion Entry **Authorized**（bounded）— Implementation Authorization **Not Granted** — catalog registration **Not Authorized** — bounded text Mock Formal Decision **READY** preserved — Production Ready **Not Declared**（global）。Real Provider / external IO は **禁止維持**。
...
- **image-generation-mock-provider Expansion Entry Authorized** — v1.80.0 ADR-0020（bounded）; Implementation Authorization **Not Granted**; catalog registration **Not Authorized**
