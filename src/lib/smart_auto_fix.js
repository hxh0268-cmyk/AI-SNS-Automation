import fs from "node:fs/promises";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { getCarouselPromptFileName } from "./carousel.js";
import { classifyRootCause } from "./root_cause.js";

const __dirname = path.dirname(fileURLToPath(import.meta.url));

/** プロジェクトルート（src/lib 基準） */
export const PROJECT_ROOT = path.resolve(__dirname, "../..");

/** デフォルトの image_review.json パス（プロジェクト相対） */
export const DEFAULT_REVIEW_JSON_PATH = "images/carousel/review/image_review.json";

/** デフォルト合格ライン */
export const DEFAULT_PASSING_SCORE = 80;

/** Smart Auto Fix 追記ブロック見出し */
export const SMART_AUTO_FIX_HEADING = "## Smart Auto Fix 指示";

/** @typedef {"TEXT" | "LAYOUT" | "PROMPT" | "STYLE" | "OTHER"} RootCause */

/**
 * @typedef {object} FixPlan
 * @property {string[]} filesToModify
 * @property {string[]} strategy
 */

/**
 * @typedef {object} SmartAutoFixTargetInfo
 * @property {string} slideKey
 * @property {number} slideNumber
 * @property {string | null} type
 * @property {number | string | null} score
 * @property {ReturnType<typeof classifyRootCause>} classification
 * @property {string} promptPath
 * @property {string | null} slideMdPath
 */

/**
 * @typedef {object} SlideProcessResult
 * @property {string} slideKey
 * @property {string | null} type
 * @property {number | string | null} score
 * @property {ReturnType<typeof classifyRootCause>} classification
 * @property {FixPlan} fixPlan
 * @property {string} promptPath
 * @property {string | null} slideMdPath
 * @property {"planned" | "applied" | "skipped" | "failed"} status
 * @property {string[]} changedFiles
 * @property {string[]} skippedFiles
 * @property {string[]} backedUpFiles
 */

/** rootCause ごとの追記指示 */
const APPLY_INSTRUCTIONS = {
  TEXT: [
    "- 画像内テキストを短くする",
    "- 日本語は誤字が出やすいため短文にする",
    "- 重要文字は大きく中央配置",
    "- EXACT text を使う",
  ],
  LAYOUT: [
    "- 文字エリアには背景模様を入れない",
    "- 背景と文字のコントラストを強くする",
    "- 余白を20%以上確保する",
    "- 文字背後に単色矩形を置く",
  ],
  PROMPT: [
    "- EXACT text 指定を追加",
    "- safe margin を明記",
    "- font size を大きく指定",
    "- no extra text を明記",
  ],
  STYLE: [
    "- ブランドカラーを統一",
    "- アイコンの雰囲気を統一",
    "- フォントイメージを統一",
    "- シリーズ感を出す",
  ],
  OTHER: [
    "- 手動確認が必要",
    "- image_review.md を確認する",
  ],
};

/**
 * @param {string} [projectRoot]
 * @returns {string}
 */
function resolveProjectRoot(projectRoot = PROJECT_ROOT) {
  return path.resolve(projectRoot);
}

/**
 * @param {string} relativeOrAbsolute
 * @param {string} projectRoot
 * @returns {string}
 */
function toAbsolutePath(relativeOrAbsolute, projectRoot) {
  return path.isAbsolute(relativeOrAbsolute)
    ? path.normalize(relativeOrAbsolute)
    : path.join(projectRoot, relativeOrAbsolute);
}

/**
 * failedItems を Set として取得する
 * @param {unknown} failedItems
 * @returns {Set<string>}
 */
export function toFailedItemSet(failedItems) {
  if (!Array.isArray(failedItems)) {
    return new Set();
  }

  return new Set(
    failedItems.filter((item) => typeof item === "string" && item.trim()),
  );
}

/**
 * 改善対象スライドか判定する
 * @param {object} slide
 * @param {Set<string>} failedItems
 * @param {number} passingScore
 * @returns {boolean}
 */
export function isSmartAutoFixTargetSlide(slide, failedItems, passingScore) {
  const slideKey = slide.fileName?.replace(/\.png$/i, "");
  if (slideKey && failedItems.has(slideKey)) {
    return true;
  }

  const score = Number(slide.score);
  return Number.isFinite(score) && score < passingScore;
}

/**
 * スライド番号を取得する
 * @param {object} slide
 * @returns {number}
 */
export function getSlideNumber(slide) {
  if (Number.isFinite(slide.number)) {
    return Number(slide.number);
  }

  const match = slide.fileName?.match(/slide(\d{2})\.png/i);
  return match ? Number(match[1]) : 0;
}

/**
 * スライド関連ファイルの相対パスを取得する
 * @param {number} slideNumber
 * @returns {{ slideMd: string, promptMd: string }}
 */
export function getSlideFilePaths(slideNumber) {
  const suffix = String(slideNumber).padStart(2, "0");
  return {
    slideMd: `content/carousel/slide${suffix}.md`,
    promptMd: `images/carousel/generated-prompts/${getCarouselPromptFileName(slideNumber)}`,
  };
}

/**
 * rootCause に応じた改善方針を返す
 * @param {RootCause} rootCause
 * @param {number} slideNumber
 * @returns {FixPlan}
 */
export function buildFixPlanForRootCause(rootCause, slideNumber) {
  const { slideMd, promptMd } = getSlideFilePaths(slideNumber);

  switch (rootCause) {
    case "TEXT":
      return {
        filesToModify: [slideMd, promptMd],
        strategy: [
          `${slideMd} の文言を短くし、誤字・文字崩れが起きにくい表現に調整する`,
          `${promptMd} に EXACT text 指定と文字崩れ防止ルール（白文字・大きい字号・無地背景）を追加する`,
        ],
      };
    case "LAYOUT":
      return {
        filesToModify: [promptMd],
        strategy: [
          `${promptMd} に余白（safe zone 20%）・中央配置・文字背後の無地矩形を追加する`,
          `${promptMd} に背景パターンが文字と重ならないよう禁止事項を追加する`,
          `${promptMd} にコントラスト強化（白文字 + 暗い背景）を明記する`,
        ],
      };
    case "PROMPT":
      return {
        filesToModify: [promptMd],
        strategy: [
          `${promptMd} に EXACT text（正確な日本語表示）指定を追加する`,
          `${promptMd} に文字サイズ（very large）と安全余白（wide margins）を追加する`,
          `${promptMd} に文字エリアを無地矩形で固定する指示を追加する`,
        ],
      };
    case "STYLE":
      return {
        filesToModify: [promptMd],
        strategy: [
          `${promptMd} に他スライドと揃えるブランドカラー・統一感の指示を追加する`,
          `${promptMd} にシリーズ全体のトーン＆マナーを合わせる指示を追加する`,
          `${promptMd} に必要に応じてアイコン・視覚要素の追加指示を記載する`,
        ],
      };
    default:
      return {
        filesToModify: [promptMd],
        strategy: [
          "自動分類できないため、images/carousel/review/image_review.md を確認して手動で対応してください",
          `${promptMd} に手動確認の指示を追記する`,
        ],
      };
  }
}

/**
 * rootCause 用の追記ブロックを生成する
 * @param {RootCause} rootCause
 * @returns {string}
 */
export function buildInstructionBlock(rootCause) {
  const lines = APPLY_INSTRUCTIONS[rootCause] ?? APPLY_INSTRUCTIONS.OTHER;
  return `${SMART_AUTO_FIX_HEADING}\n\n${lines.join("\n")}\n`;
}

/**
 * スライドの rootCause 分類とメタ情報を返す
 * @param {object} slide
 * @returns {SmartAutoFixTargetInfo}
 */
export function classifySmartAutoFixTarget(slide) {
  const slideNumber = getSlideNumber(slide);
  const slideKey =
    slide.fileName ?? `slide${String(slideNumber).padStart(2, "0")}.png`;
  const classification = classifyRootCause(slide);
  const filePaths = getSlideFilePaths(slideNumber);

  return {
    slideKey,
    slideNumber,
    type: slide.type ?? null,
    score: slide.score ?? null,
    classification,
    promptPath: filePaths.promptMd,
    slideMdPath: filePaths.slideMd,
  };
}

/**
 * 1 スライド分の Smart Auto Fix 計画を組み立てる
 * @param {object} slide
 * @param {object} [options]
 * @returns {SmartAutoFixTargetInfo & { fixPlan: FixPlan }}
 */
export function buildSmartAutoFixPlan(slide, options = {}) {
  const targetInfo = classifySmartAutoFixTarget(slide);
  const fixPlan = buildFixPlanForRootCause(
    targetInfo.classification.rootCause,
    targetInfo.slideNumber,
  );

  return {
    ...targetInfo,
    fixPlan,
  };
}

/**
 * レポート用タイムスタンプを生成する
 * @param {Date} [date]
 * @returns {string}
 */
export function formatReportTimestamp(date = new Date()) {
  const year = date.getFullYear();
  const month = String(date.getMonth() + 1).padStart(2, "0");
  const day = String(date.getDate()).padStart(2, "0");
  const hours = String(date.getHours()).padStart(2, "0");
  const minutes = String(date.getMinutes()).padStart(2, "0");
  const seconds = String(date.getSeconds()).padStart(2, "0");
  return `${year}-${month}-${day}-${hours}${minutes}${seconds}`;
}

/**
 * 実行結果ラベルを決める
 * @param {object} params
 * @returns {string}
 */
export function determineSmartAutoFixResult({ targetCount, applyMode, slideResults }) {
  if (targetCount === 0) {
    return "改善対象なし";
  }

  const hasOther = slideResults.some(
    (item) => item.classification.rootCause === "OTHER",
  );
  if (hasOther) {
    return "手動確認が必要";
  }

  if (applyMode) {
    return "apply完了";
  }

  return "dry-run完了";
}

/**
 * レポート Markdown を生成する
 * @param {object} data
 * @returns {string}
 */
export function buildSmartAutoFixReportMarkdown(data) {
  const lines = [
    "# Smart Auto Fix Report",
    "",
    "## 実行概要",
    "",
    `- 実行日時: ${data.executedAt}`,
    `- 実行モード: ${data.mode}`,
    `- 画像レビュー総合 score: ${data.reviewScore ?? "不明"}`,
    `- passed: ${data.passed ?? "不明"}`,
    `- failedItems: ${data.failedItemsText}`,
    `- 結果: **${data.result}**`,
    "",
  ];

  if (data.targetSlides.length === 0) {
    lines.push("## 改善対象スライド", "", "なし（すべて合格ライン以上）", "");
  } else {
    lines.push("## 改善対象スライド", "");
    for (const slide of data.targetSlides) {
      lines.push(`### ${slide.slideKey}`);
      lines.push("");
      lines.push(`- 種別: ${slide.type ?? "不明"}`);
      lines.push(`- score: ${slide.score ?? "不明"}`);
      lines.push(`- rootCause: ${slide.classification.rootCause}`);
      lines.push(`- reason: ${slide.classification.reason}`);
      lines.push(
        `- matchedKeywords: ${
          slide.classification.matchedKeywords.length > 0
            ? slide.classification.matchedKeywords.join(", ")
            : "なし"
        }`,
      );
      lines.push("- 修正予定ファイル:");
      if (slide.fixPlan.filesToModify.length > 0) {
        for (const filePath of slide.fixPlan.filesToModify) {
          lines.push(`  - ${filePath}`);
        }
      } else {
        lines.push("  - なし");
      }
      lines.push("- 改善方針:");
      for (const strategy of slide.fixPlan.strategy) {
        lines.push(`  - ${strategy}`);
      }
      lines.push("");
    }
  }

  lines.push("## 実行結果（ファイル）", "");
  lines.push("### 実際に変更したファイル");
  if (data.changedFiles.length > 0) {
    for (const filePath of data.changedFiles) {
      lines.push(`- ${filePath}`);
    }
  } else {
    lines.push("- なし");
  }
  lines.push("");
  lines.push("### バックアップしたファイル");
  if (data.backedUpFiles.length > 0) {
    for (const filePath of data.backedUpFiles) {
      lines.push(`- ${filePath}`);
    }
  } else {
    lines.push("- なし");
  }
  lines.push("");

  return `${lines.join("\n")}\n`;
}

/**
 * image_review.json を読み込む
 * @param {object} [options]
 * @param {string} [options.reviewPath]
 * @param {string} [options.projectRoot]
 * @returns {Promise<object>}
 */
export async function loadReviewJson(options = {}) {
  const projectRoot = resolveProjectRoot(options.projectRoot);
  const reviewPath = options.reviewPath ?? DEFAULT_REVIEW_JSON_PATH;
  const absolutePath = toAbsolutePath(reviewPath, projectRoot);

  let fileContent;
  try {
    fileContent = await fs.readFile(absolutePath, "utf-8");
  } catch (error) {
    if (error && typeof error === "object" && "code" in error && error.code === "ENOENT") {
      throw new Error(
        `レビューファイルが見つかりません: ${reviewPath}`,
      );
    }
    const message = error instanceof Error ? error.message : String(error);
    throw new Error(`image_review.json の読み込みに失敗しました: ${message}`);
  }

  try {
    return JSON.parse(fileContent);
  } catch {
    throw new Error("image_review.json の形式が不正です。");
  }
}

/**
 * ファイルが存在するか確認する
 * @param {string} absolutePath
 * @returns {Promise<boolean>}
 */
async function fileExists(absolutePath) {
  try {
    const stat = await fs.stat(absolutePath);
    return stat.isFile();
  } catch {
    return false;
  }
}

/**
 * apply モード用に対象ファイルをバックアップする
 * @param {string} relativePath
 * @param {string} projectRoot
 * @returns {Promise<string | null>}
 */
async function backupFile(relativePath, projectRoot) {
  const sourcePath = path.join(projectRoot, relativePath);
  const exists = await fileExists(sourcePath);
  if (!exists) {
    return null;
  }

  const backupDir = path.join(projectRoot, "images/carousel/backup");
  await fs.mkdir(backupDir, { recursive: true });

  const backupName = `${path.basename(relativePath, path.extname(relativePath))}-before-smart-auto-fix${path.extname(relativePath)}`;
  const backupPath = path.join(backupDir, backupName);
  await fs.copyFile(sourcePath, backupPath);
  return `images/carousel/backup/${backupName}`;
}

/**
 * Smart Auto Fix 指示をファイル末尾に追記する
 * @param {string} relativePath
 * @param {RootCause} rootCause
 * @param {string} projectRoot
 * @returns {Promise<"changed" | "skipped" | "missing">}
 */
async function appendInstructions(relativePath, rootCause, projectRoot) {
  const absolutePath = path.join(projectRoot, relativePath);
  const exists = await fileExists(absolutePath);
  if (!exists) {
    return "missing";
  }

  const content = await fs.readFile(absolutePath, "utf-8");
  if (content.includes(SMART_AUTO_FIX_HEADING)) {
    return "skipped";
  }

  const block = buildInstructionBlock(rootCause);
  const newContent = `${content.trimEnd()}\n\n${block}`;
  await fs.writeFile(absolutePath, newContent);
  return "changed";
}

/**
 * スライド詳細ログ行を生成する
 * @param {SmartAutoFixTargetInfo & { fixPlan: FixPlan }} plan
 * @returns {string[]}
 */
function buildSlideDetailLogLines(plan) {
  const lines = [
    `【対象スライド】 ${plan.slideKey}`,
    `- 種別: ${plan.type ?? "不明"}`,
    `- score: ${plan.score ?? "不明"}`,
    `- rootCause: ${plan.classification.rootCause}`,
    `- reason: ${plan.classification.reason}`,
  ];

  if (plan.classification.matchedKeywords.length > 0) {
    lines.push(
      `- matchedKeywords: ${plan.classification.matchedKeywords.join(", ")}`,
    );
  } else {
    lines.push("- matchedKeywords: なし");
  }

  if (plan.fixPlan.filesToModify.length > 0) {
    lines.push("- 修正予定ファイル:");
    for (const filePath of plan.fixPlan.filesToModify) {
      lines.push(`  - ${filePath}`);
    }
  } else {
    lines.push("- 修正予定ファイル: なし（手動確認）");
  }

  lines.push("- 改善方針:");
  for (const strategy of plan.fixPlan.strategy) {
    lines.push(`  - ${strategy}`);
  }

  return lines;
}

/**
 * apply 結果ログ行を生成する
 * @param {object} params
 * @returns {string[]}
 */
function buildApplyResultLogLines(params) {
  const { changedFiles, skippedFiles, backedUpFiles } = params;
  const lines = [];

  if (backedUpFiles.length > 0) {
    lines.push("- バックアップ:");
    for (const backupPath of backedUpFiles) {
      lines.push(`  - ${backupPath}`);
    }
  }

  if (changedFiles.length > 0) {
    lines.push("- apply mode: 指示を追記しました");
    lines.push("- 変更したファイル:");
    for (const filePath of changedFiles) {
      lines.push(`  - ${filePath}`);
    }
  } else if (skippedFiles.length > 0) {
    lines.push("- apply mode: 追記スキップ（既に Smart Auto Fix 指示あり）");
    for (const filePath of skippedFiles) {
      lines.push(`  - ${filePath}`);
    }
  } else {
    lines.push("- apply mode: 変更対象ファイルが見つかりませんでした");
  }

  return lines;
}

/**
 * 1 スライド分の Smart Auto Fix を計画または apply する
 * @param {object} slide
 * @param {object} [options]
 * @param {boolean} [options.apply=false]
 * @param {string} [options.projectRoot]
 * @param {(message: string) => void} [options.log]
 * @returns {Promise<SlideProcessResult>}
 */
export async function applySmartAutoFixForSlide(slide, options = {}) {
  const apply = options.apply ?? false;
  const projectRoot = resolveProjectRoot(options.projectRoot);
  const log = options.log ?? (() => {});

  const plan = buildSmartAutoFixPlan(slide, options);
  const changedFiles = [];
  const skippedFiles = [];
  const backedUpFiles = [];

  for (const line of buildSlideDetailLogLines(plan)) {
    log(line);
  }

  if (apply && plan.fixPlan.filesToModify.length > 0) {
    for (const relativePath of plan.fixPlan.filesToModify) {
      const backupPath = await backupFile(relativePath, projectRoot);
      if (backupPath) {
        backedUpFiles.push(backupPath);
      }

      const result = await appendInstructions(
        relativePath,
        plan.classification.rootCause,
        projectRoot,
      );

      if (result === "changed") {
        changedFiles.push(relativePath);
      } else if (result === "skipped") {
        skippedFiles.push(relativePath);
      }
    }

    for (const line of buildApplyResultLogLines({
      changedFiles,
      skippedFiles,
      backedUpFiles,
    })) {
      log(line);
    }
  }

  log("");

  let status = "planned";
  if (apply) {
    if (changedFiles.length > 0) {
      status = "applied";
    } else if (skippedFiles.length > 0) {
      status = "skipped";
    } else {
      status = "failed";
    }
  }

  return {
    slideKey: plan.slideKey,
    type: plan.type,
    score: plan.score,
    classification: plan.classification,
    fixPlan: plan.fixPlan,
    promptPath: plan.promptPath,
    slideMdPath: plan.slideMdPath,
    status,
    changedFiles,
    skippedFiles,
    backedUpFiles,
  };
}

/**
 * レポートを保存する
 * @param {string} content
 * @param {Date} executedAt
 * @param {object} [options]
 * @returns {Promise<string>}
 */
export async function saveSmartAutoFixReport(content, executedAt, options = {}) {
  const projectRoot = resolveProjectRoot(options.projectRoot);
  const reportDir = path.join(projectRoot, "reports/smart-auto-fix");
  await fs.mkdir(reportDir, { recursive: true });
  const fileName = `${formatReportTimestamp(executedAt)}.md`;
  const relativePath = `reports/smart-auto-fix/${fileName}`;
  const absolutePath = path.join(projectRoot, relativePath);
  await fs.writeFile(absolutePath, content);
  return relativePath;
}

/**
 * Smart Auto Fix を一括実行する
 * @param {object} [options]
 * @param {boolean} [options.apply=false]
 * @param {string} [options.reviewPath]
 * @param {string} [options.projectRoot]
 * @param {number} [options.passingScore]
 * @param {(message: string) => void} [options.log]
 * @returns {Promise<object>}
 */
export async function runSmartAutoFix(options = {}) {
  const apply = options.apply ?? false;
  const log = options.log ?? (() => {});
  const executedAt = new Date();

  log("========================================");
  log("Smart Auto Fix");
  log("========================================");
  log("");
  log(
    apply
      ? "モード: apply（バックアップ後に Smart Auto Fix 指示を追記します）"
      : "モード: dry-run（計画表示のみ・ファイルは変更しません）",
  );
  log("");

  const review = await loadReviewJson(options);
  const passingScore = Number(
    options.passingScore ?? review.passingScore ?? DEFAULT_PASSING_SCORE,
  );
  const failedItems = toFailedItemSet(review.failedItems);
  const failedItemsList = [...failedItems];
  const slides = Array.isArray(review.slides) ? review.slides : [];

  const targets = slides.filter((slide) =>
    isSmartAutoFixTargetSlide(slide, failedItems, passingScore),
  );

  log(`合格ライン: ${passingScore} 点`);
  log(
    `failedItems: ${failedItemsList.length > 0 ? failedItemsList.join(", ") : "なし"}`,
  );
  log(
    `総合判定: ${review.passed === true ? "合格" : review.passed === false ? "不合格" : "不明"}`,
  );
  log("");

  const slideResults = [];
  const allChangedFiles = new Set();
  const allBackedUpFiles = new Set();

  if (targets.length === 0) {
    log("改善対象なし");
    log("");
    log("すべてのスライドが合格ライン以上です。");
    log("");
  } else {
    log(`改善対象: ${targets.length} 枚`);
    log("");

    for (const slide of targets) {
      const result = await applySmartAutoFixForSlide(slide, {
        ...options,
        apply,
        log,
      });
      slideResults.push(result);

      for (const filePath of result.changedFiles) {
        allChangedFiles.add(filePath);
      }
      for (const filePath of result.backedUpFiles) {
        allBackedUpFiles.add(filePath);
      }
    }
  }

  const resultLabel = determineSmartAutoFixResult({
    targetCount: targets.length,
    applyMode: apply,
    slideResults,
  });

  const reportPath = await saveSmartAutoFixReport(
    buildSmartAutoFixReportMarkdown({
      executedAt: executedAt.toLocaleString("ja-JP"),
      mode: apply ? "apply" : "dry-run",
      reviewScore: review.score ?? null,
      passed: review.passed ?? null,
      failedItemsText:
        failedItemsList.length > 0 ? failedItemsList.join(", ") : "なし",
      result: resultLabel,
      targetSlides: slideResults,
      changedFiles: [...allChangedFiles],
      backedUpFiles: [...allBackedUpFiles],
    }),
    executedAt,
    options,
  );

  log("========================================");
  log("Smart Auto Fix 完了");
  log("========================================");

  if (apply && allChangedFiles.size > 0) {
    log("");
    log("【変更したファイル一覧】");
    for (const filePath of allChangedFiles) {
      log(`- ${filePath}`);
    }
    log("");
    log("（画像生成・画像レビューはまだ実行しません）");
  }

  if (!apply && targets.length > 0) {
    log("");
    log("指示を追記する場合:");
    log("npm run smart-auto-fix -- --apply");
  }

  log("");
  log("【レポート保存】");
  log(`→ ${reportPath}`);
  log(`   結果: ${resultLabel}`);

  return {
    apply,
    passingScore,
    targetCount: targets.length,
    slideResults,
    changedFiles: [...allChangedFiles],
    backedUpFiles: [...allBackedUpFiles],
    reportPath,
    result: resultLabel,
    review,
  };
}
